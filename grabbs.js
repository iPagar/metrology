...
function showResult() {
  ...
  const kvantilStudenta = prompt(
    "Введите квантиль стьюдента при " + "n = " + values.length + ", p = 0.95"
  );
  ...
  if (checkCorrection()) {
    ...
    const skoSum = Math.sqrt(sdX() * sdX() + sdSystem() * sdSystem()).toFixed(
      decimals
    );
    const delta = (k * skoSum).toFixed(decimals);

    container.innerHTML +=
      katex.renderToString("x = \\bar{x} \\pm \\Delta") + ", p = " + p + "<br>";
    container.innerHTML +=
      "n = " +
      values.length +
      ", p = 0.95 =>" +
    ...
    container.innerHTML +=
      katex.renderToString(
        "S_{\\sum} = \\sqrt{S_{\\bar{x}}^2 + S_{\\Theta}^2} = \\sqrt{" +
          Math.pow(sdX(), 2).toFixed(decimals) +
          "^2 + " +
          Math.pow(sdSystem(), 2).toFixed(decimals) +
          "^2}=" +
          skoSum
      ) + "<br>";
    container.innerHTML +=
  ...
  if (checkCorrection()) {
    container.innerHTML +=
      katex.renderToString(
        Math.abs(system) +
          ">" +
          Number(0.5 * (Math.pow(sdSystem(), 2) / sd())).toFixed(decimals)
...
function createFirst() {
  ...
  let formulaFirst =
    katex.renderToString(
      "\\bar{x} = \\frac{1}{n}\\displaystyle\\sum_{i=1}^nx_{i} = \\frac{1}{" +
        values.length +
        "}(" +
        values[0] +
        " + ... +" +
  ...
  let formulaSecond =
    katex.renderToString(
      "S_x = \\sqrt{\\frac{1}{n - 1}\\displaystyle\\sum_{i=1}^n(x_{i} - \\bar{x})^{2}} = \\sqrt{\\frac{1}{" +
        (values.length - 1) +
        "}(" +
        values[0] +
        "-" +
  ...
  first.innerHTML =
    ...
    repeat +
    ") Вычисляем x̅ и S<sub>x</sub> результатов измерений</div> <div>" +
    formulaFirst +
    "</div>" +
    "<div>" +
    formulaSecond +
...
function createSecond() {
  ...
  let formulaFirst =
    katex.renderToString("x_{max} = " + max + "(" + dimension + ")") +
    katex.renderToString(
      " => G_{max} = \\frac{|x_{max} - \\bar{x}|}{S_x} = \\frac{|" +
        max +
        "-" +
        avg() +
        "|}{" +
  ...
  if (gMax() > grabbs)
    formulaFirst +=
      katex.renderToString("~=>~G_{max} > G_t~=>~x_{max}") +
      "содержит грубую погрешность";
  else
    formulaFirst +=
      katex.renderToString(" => G_{max} \\le G_t~=>~x_{max}") +
      "не содержит грубую погрешность";
  let formulaSecond =
    katex.renderToString("x_{min} = " + min + "(" + dimension + ")") +
    katex.renderToString(
      "~=>~G_{min} = \\frac{|x_{min} - \\bar{x}|}{S_x} = \\frac{|" +
        min +
        "-" +
        avg() +
        "|}{" +
  ...
  if (gMin() > grabbs)
    formulaSecond +=
      katex.renderToString(" => G_{min} > G_t~=>~x_{min}") +
      "содержит грубую погрешность";
  else
    formulaSecond +=
      katex.renderToString(" => G_{min} \\le G_t~=>~x_{min}") +
      "не содержит грубую погрешность";
  first.innerHTML =
    '<div style="display: block">' +
    ++repeat +
    ") Выявляем результаты, содержащие грубые погрешности</div><div>n = " +
    values.length +
    ", q = " +
    q +
    ", => G<sub>t</sub> = " +
...
function calc() {
  g();
  if (system && borderSystem) showCorrection();
  showResult();
}
